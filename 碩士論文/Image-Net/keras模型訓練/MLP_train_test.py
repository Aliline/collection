...
savepath = "D:/F111156114_Aliline/imgtoword/wordtovector/MLP_renew120/"
testpath = "D:/F111156114_Aliline/imgtoword/wordtovector/MLP_renew120/"
xvector = np.load('D:/F111156114_Aliline/imgtoword/wordtovector/vector_MLP_renew/vx120_100.npy',allow_pickle=True)
yvector = np.load('D:/F111156114_Aliline/imgtoword/wordtovector/vector_MLP_renew/v120y_100.npy')
testtxt = "D:/F111156114_Aliline/imgtoword/wordtovector/vector_txt/vectors_all_ver_.txt"
testvecotr = np.load("D:/F111156114_Aliline/imgtoword/wordtovector/vectors_120_ver2.npy")
actiname = 'tanh'
lr=0.09
beta_1=0.9
beta_2=0.999
...
classes=120
while True:
    if not os.path.exists(f'{savepath}MLP_ver{ver_num}_{actiname}_{peroid}.h5'):
        ver = ver_num
        break
    ver_num = ver_num +1
...
#activation='elu'
    ...
    input1 = Input(shape=(img_shape[1]))
    ...
    # h= BatchNormalization()(h)
    # h = Dropout(0.30)(h)
    h = Dense(4096,activation=actiname,kernel_regularizer=regularizers.l2(0.01))(input1)
    h= BatchNormalization()(h)
    h = Dropout(0.50)(h)
    h = Dense(2048,activation=actiname,kernel_regularizer=regularizers.l2(0.01))(h)
    h= BatchNormalization()(h)
    h = Dropout(0.50)(h)
    ...
    model.compile(loss='mean_squared_error',
              optimizer=keras.optimizers.Adagrad(lr=lr,epsilon = epsilon),
...
# optimizer=keras.optimizers.Adagrad(lr=lr)
    ...
    savepoint_path = f"{savepath}MLP_{classes}_ver{ver}_{actiname}_"
    ...
    model.save(f'{savepath}MLP_{classes}_ver{ver}_complete_{actiname}.h5')
...
x_tran,x_test,y_train,y_test = train_test_split(xvector, yvector, test_size=0.90)
temp = x_tran.shape
img_shape = [temp[0],temp[1]]
# reset_keras()
model_name = []
Acc_set = []
...
for model in os.listdir(testpath) :
    ...
    xvector = np.load('D:/F111156114_Aliline/imgtoword/wordtovector/vector_MLP_renew/vx120_full.npy',allow_pickle=True)
    yvector = np.load('D:/F111156114_Aliline/imgtoword/wordtovector/vector_MLP_renew/v{}y_full.npy'.format(classes))
